...
knitr::kable(SSID_summary, caption = "All SSIDs and their counts from Event_log.csv")
```
...
-   Clients that has **device names** in them, such as `MACs-MacBook-Pro-2` and `Android-5`, will be grouped into their respective devices, so `MACs-MacBook-Pro-2` belongs to `Mac`, whereas `Android-5` belongs to `Android`.
